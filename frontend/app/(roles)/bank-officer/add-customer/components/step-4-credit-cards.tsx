...
const DEFAULT_PROVIDER = "Standard Card";

export function CreditCards({ formData, updateFormData, onNext, onBack, isSavingFinancialStep }: StepProps) {
  const [creditLimit, setCreditLimit] = useState("");
  const [outstandingBalance, setOutstandingBalance] = useState("");
  const [errors, setErrors] = useState<CreditCardStepErrors>({});

  const handleAddCard = () => {
    const draftErrors = validateCreditCardDraft({ issuer: DEFAULT_PROVIDER, limit: creditLimit, outstandingBalance });
    ...
    updateFormData({
      creditCards: [...formData.creditCards, { issuer: DEFAULT_PROVIDER, limit: creditLimit.trim(), outstandingBalance: outstandingBalance.trim() }],
    });
    setCreditLimit("");
  ...
  const handleNext = () => {
    ...
    const hasPendingCard = Boolean(creditLimit.trim() || outstandingBalance.trim());
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
      <div className="px-8 py-6 border-b border-slate-100">
        ...
        <p className="text-sm text-slate-500 mt-1">Enter details of the customer&apos;s current active credit cards.</p>
      </div>
      ...
      <div className="p-8 space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-3">
            ...
            <p className="text-[10px] text-slate-400">Total sanctioned limit of the card.</p>
          </div>
          <div className="space-y-3">
            ...
            <p className="text-[10px] text-slate-400">Current amount due as per latest statement.</p>
          </div>
        </div>
