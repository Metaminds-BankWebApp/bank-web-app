...
import { getBankCreditTrends } from "@/src/api/creditlens/bank-creditlens.service";
import type { CreditTrendResponse } from "@/src/types/dto/bank-creditlens.dto";
  ...
  useEffect(() => {
    ...
    const loadTrends = async () => {
      try {
        setIsLoading(true);
        setError(null);
        const response = await getBankCreditTrends(trendRange);
        if (isActive) {
      ...
      <div className="flex min-h-[calc(100dvh-1.25rem)] flex-col gap-4 sm:gap-5 lg:h-[calc(100dvh-2rem)] lg:min-h-0 lg:overflow-hidden">
        ...
        {isLoading && !trendData ? (
          ...
          <StateCard
            ...
            description="Once more monthly evaluations are available, your CreditLens trend history will appear here."
          />
        )}
      </div>
    </div>
