...
type SpendIqRiskLabel = "Healthy" | "Moderate" | "Risky" | "Very Risky";
...
function getSpendIqRiskLabel(score: number): SpendIqRiskLabel {
...
function getSpendIqRiskClass(label: SpendIqRiskLabel) {
  if (label === "Healthy") return "text-emerald-600";
  if (label === "Moderate") return "text-amber-500";
  if (label === "Risky") return "text-orange-500";
  return "text-red-500";
}
...
function buildSpendIqPdfReport({
  ...
  const pageTwo: string[] = [
    ...
    text("Why this score?", 36, 104, 15, "0.04 0.10 0.23", "F2"),
  ];

  let y = 126;
  scoreReasons.forEach((reason, index) => {
    pageTwo.push(rect(36, y, 523, 58, "0.98 0.99 1.00"));
    pageTwo.push(rect(36, y, 523, 58, "0.84 0.89 0.95", true));
    pageTwo.push(text(`${index + 1}. ${reason.factor}: -${reason.impact}`, 54, y + 20, 10, "0.04 0.10 0.23", "F2"));
    pageTwo.push(text(reason.detail.slice(0, 95), 54, y + 40, 8, "0.30 0.36 0.45"));
    y += 70;
  });
  ...
  const pageThree: string[] = [
    rect(0, 0, pageWidth, 70, "0.04 0.10 0.23"),
    text("SpendIQ Category Detail", 36, 44, 19, "1 1 1", "F2"),
    text(selectedMonthLabel, 430, 44, 11, "0.78 0.86 0.96", "F2"),
    text("All Category Details", 36, 104, 15, "0.04 0.10 0.23", "F2"),
  ];

  y = 126;
  pageThree.push(rect(36, y, 523, 28, "0.91 0.95 0.99"));
  pageThree.push(text("Category", 48, y + 18, 8, "0.20 0.25 0.34", "F2"));
  pageThree.push(text("Type", 230, y + 18, 8, "0.20 0.25 0.34", "F2"));
  pageThree.push(text("Records", 318, y + 18, 8, "0.20 0.25 0.34", "F2"));
  pageThree.push(text("Total", 430, y + 18, 8, "0.20 0.25 0.34", "F2"));
  y += 32;

  categoryRows.slice(0, 14).forEach((row) => {
    pageThree.push(line(36, y + 18, 559, y + 18, "0.88 0.91 0.95"));
    pageThree.push(text(row.name.slice(0, 28), 48, y + 12, 8));
    pageThree.push(text(row.type, 230, y + 12, 8));
    pageThree.push(text(row.count, 330, y + 12, 8));
    pageThree.push(text(money(row.amount), 430, y + 12, 8, "0.04 0.10 0.23", "F2"));
    y += 24;
  });

  if (categoryRows.length === 0) {
    pageThree.push(text("No category records available for this period.", 48, y + 12, 9, "0.39 0.45 0.55"));
  }

  const pageFour: string[] = [
    rect(0, 0, pageWidth, 70, "0.04 0.10 0.23"),
  ...
  y = 126;
  pageFour.push(rect(36, y, 523, 28, "0.91 0.95 0.99"));
  pageFour.push(text("Date", 48, y + 18, 8, "0.20 0.25 0.34", "F2"));
  pageFour.push(text("Category", 122, y + 18, 8, "0.20 0.25 0.34", "F2"));
  pageFour.push(text("Payment", 280, y + 18, 8, "0.20 0.25 0.34", "F2"));
  pageFour.push(text("Amount", 440, y + 18, 8, "0.20 0.25 0.34", "F2"));
  y += 32;

  expenses.slice(0, 20).forEach((expense) => {
    pageFour.push(line(36, y + 18, 559, y + 18, "0.88 0.91 0.95"));
    pageFour.push(text(expense.expenseDate, 48, y + 12, 8));
    pageFour.push(text(expense.categoryName.slice(0, 24), 122, y + 12, 8));
    pageFour.push(text(String(expense.paymentType).replace("_", " "), 280, y + 12, 8));
    pageFour.push(text(money(Number(expense.amount)), 440, y + 12, 8, "0.86 0.15 0.15", "F2"));
    y += 24;
  });

  if (expenses.length === 0) {
    pageFour.push(text("No expense records available for this period.", 48, y + 12, 9, "0.39 0.45 0.55"));
  }
...
export function SpendIqReportPage({ title = "SpendIQ - Analytics Report" }: SpendIqReportPageProps) {
  ...
  const scoreLabel = getSpendIqRiskLabel(spendIqScore);
  const scoreLabelClass = getSpendIqRiskClass(scoreLabel);
  const scoreReasons = useMemo<SpendIqScoreReason[]>(() => {
    ...
    if (budgetPenalty > 0) {
      reasons.push({
        ...
        detail: summary.totalBudget <= 0
          ? "No budget is set for this report period, so SpendIQ cannot confirm spending is controlled."
          : `Budget usage is ${budgetUsage.toFixed(2)}%, which reduces the score when usage is high or over the limit.`,
      });
    }
    ...
    if (savingsPenalty > 0) {
      reasons.push({
        ...
        detail: Number(summary.netSavings) < 0
          ...
          : "Savings are below 10% of recorded income, so the score is reduced.",
      });
    }
    ...
    if (concentrationPenalty > 0 && topCategory) {
      reasons.push({
        ...
        detail: `${topCategory.name} takes ${(topCategoryShare * 100).toFixed(2)}% of total spend, so spending is concentrated in one category.`,
      });
    }
    ...
    if (highValuePenalty > 0) {
      reasons.push({
        ...
        detail: `${highValueCount} transaction${highValueCount === 1 ? "" : "s"} are unusually high compared with the user's average expense.`,
      });
    }
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-3">
        <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          ...
          <div className="mt-6 text-sm text-slate-500 dark:text-slate-400">
            Risk Level: <span className={`font-medium ${scoreLabelClass}`}>{scoreLabel}</span>
          </div>
